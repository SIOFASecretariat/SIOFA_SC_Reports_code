...
ggplot() +
  ...
  ggtitle("SIOFA RIB fishing activities (5 degrees, 2014–2023)") +
...
##discards
# filter all RIB target operations (with full data)
RIB_target_operations <- fishing %>%
  filter(ActivityID %in% yearly_operations_RIB_target$ActivityID 
         & datasetID %in% yearly_operations_RIB_target$datasetID |
           (Target == "RIB"))
# load discards data
discards_species_RIB <- filter(RIB_target_operations, catchFate == "0")
# plot discards
# sort discards data for plotting
sort_discards_species_RIB <- aggregate(discards_species_RIB$CatchTon, by=list(Species=discards_species_RIB$SpeciesCode), FUN=sum, na.rm = TRUE)
sort_discards_species_RIB <- arrange(sort_discards_species_RIB, desc(x)) 
top5_discards_species_RIB <- sort_discards_species_RIB %>% slice(1:5)
other_discards_species_RIB <- sort_discards_species_RIB %>% slice(6:52)

ggplot(data= subset(discards_species_RIB, SpeciesCode %in% top5_discards_species_RIB$Species), aes(x = Year, y = CatchTon)) +
  geom_bar(aes(fill=SpeciesCode), position="stack", stat="identity") +
  geom_bar(data= subset(discards_species_RIB, SpeciesCode %in% other_discards_species_RIB$Species), aes(x = Year, y = CatchTon, fill="'Other species'"), position="stack", stat="identity") +
  theme_bw() +
...
ggplot(data= subset(discards_species_RIB, SpeciesCode %in% top5_discards_species_RIB$Species), aes(x = Year, y = CatchTon)) +
  ...
  scale_fill_oi(
    ...
    order = 8:1) +
  theme_bw() +
  labs(title="Yearly discards by species in RIB fisheries in the SIOFA area", x="Year", y="Discards (t)") +
  scale_x_continuous(limits=c(2013, 2024),breaks = seq(2013, 2024, by = 5)) +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio=0.4) 

ggsave("RIB summary/discards_species_RIB_web.png", width = 10, height = 4, dpi = 150)

# calculate discard rates
# total catch per year
catch_discards_RIB <- catch_bycatch_subarea_RIB %>% 
  dplyr::select(-SubAreaNo) %>%
  replace(is.na(.), 0) %>%
  group_by(Year) %>%
  summarise(TotCatch=sum(TargetCatch, NonTargetCatch))
# total discards per year
discards_RIB <- discards_species_RIB %>%
  group_by(Year) %>%
  summarise(TotDiscards=sum(CatchTon))
# join and correct for discards double counting
catch_discards_RIB <- full_join(catch_discards_RIB, discards_RIB)
catch_discards_RIB <- catch_discards_RIB %>%
  replace(is.na(.), 0) %>%
...
# colorblind friendly version
ggplot(catch_discards_RIB, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="fill", stat="identity") +
  labs(title="Yearly retained catch/discards in RIB fisheries in the SIOFA area (relative)", x="Year", y="Total catch proportion") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 5:6) +
...
ggsave("RIB summary/SIOFAcatch_discards_RIB_fill_web.png", width = 10, height = 4, dpi = 150)

#colorblind friendly version
ggplot(catch_discards_RIB, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="stack", stat="identity") +
  labs(title="Yearly retained catch/discards in RIB fisheries in the SIOFA area (absolute)", x="Year", y="Total catch (t)") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 4:5) +
...
ggsave("RIB summary/SIOFAcatch_discards_RIB_web.png", width = 10, height = 4, dpi = 150)
...
ggplot() +
  ...
  ggtitle("SIOFA incidental captures of seabirds, marine mammals, turtles and 
          sharks considered to be at high risk and/or of concern in RIB target operations (2014-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable
FT.captures_sharks_aggregate_RIB <- captures_sharks_aggregate_RIB %>%
  dplyr::select(1:5) %>%
  flextable() %>%
  ...
  add_header_lines("Observed captures of sharks at ‘high risk’ and ‘of concern’ in SIOFA common mora fisheries") 

save_as_image(FT.captures_sharks_aggregate_RIB, path = "RIB Summary/Tables/captures_sharks_RIB.png")               
...
ggplot() +
  ...
  ggtitle("SIOFA observations of seabirds and marine mammals
            in RIB target operations (2014-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable
FT.observations_seabirds_aggregate_RIB <- observations_seabirds_aggregate_RIB %>%
  dplyr::select(1:5) %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  flextable::align(j = 5, align = "center", part = "all") %>%
  italic(j = 3, italic = TRUE, part = "body") %>%
  add_header_lines("Observed seabirds around SIOFA common mora fishing operations") 

save_as_image(FT.observations_seabirds_aggregate_RIB, path = "RIB Summary/Tables/observations_seabirds_RIB.png")               
...
# as flextable
FT.countries_observing_RIB <- countries_observing_RIB %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Common mora observer data submitted by different SIOFA CCPs")

save_as_image(FT.countries_observing_RIB, path = "RIB summary/Tables/countries_observing_RIB.png")  
...
# as flextable
FT.measured_fish_RIB <- measured_fish_RIB %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  compose(j = 1, i = nrow(measured_fish_RIB), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(measured_fish_RIB), j = 1) %>%
  add_header_lines("Common mora observer data measurements")

save_as_image(FT.measured_fish_RIB, path = "RIB summary/Tables/measured_fish_RIB.png")  
...
# as flextable
FT.countries_fishing_RIB <- countries_fishing_RIB %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Common mora catch and effort data submitted by different SIOFA CCPs")

save_as_image(FT.countries_fishing_RIB, path = "RIB summary/Tables/countries_fishing_RIB.png") 

## analyze average number of vessels in the last 5 years
vessels_fishing_RIB <- fishing %>%
  filter(ActivityID %in% yearly_operations_RIB_target$ActivityID 
         & datasetID %in% yearly_operations_RIB_target$datasetID |
           Target == "RIB") %>%
