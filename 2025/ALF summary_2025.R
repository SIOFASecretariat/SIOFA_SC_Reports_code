...
ggplot() +
  ...
  ggtitle("SIOFA ALF fishing activities (hybrid 20'+30', 2014–2023)") +
...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA ALF fishing activities (5 degrees, 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
##discards
# filter all BYS target operations (with full data)
BYS_target_operations <- fishing %>%
  filter(ActivityID %in% yearly_operations_BYS_target$ActivityID 
         & datasetID %in% yearly_operations_BYS_target$datasetID |
           Target == "BYS")
# load discards data
discards_species_BYS <- filter(BYS_target_operations, catchFate == "0")
# plot discards
# sort discards data for plotting
sort_discards_species_BYS <- aggregate(discards_species_BYS$CatchTon, by=list(Species=discards_species_BYS$SpeciesCode), FUN=sum, na.rm = TRUE)
sort_discards_species_BYS <- arrange(sort_discards_species_BYS, desc(x)) 
top5_discards_species_BYS <- sort_discards_species_BYS %>% slice(1:5)
other_discards_species_BYS <- sort_discards_species_BYS %>% slice(6:52)

ggplot(data= subset(discards_species_BYS, SpeciesCode %in% top5_discards_species_BYS$Species), aes(x = Year, y = CatchTon)) +
  geom_bar(aes(fill=SpeciesCode), position="stack", stat="identity") +
  geom_bar(data= subset(discards_species_BYS, SpeciesCode %in% other_discards_species_BYS$Species), aes(x = Year, y = CatchTon, fill="'Other species'"), position="stack", stat="identity") +
  theme_bw() +
...
ggplot(data= subset(discards_species_BYS, SpeciesCode %in% top5_discards_species_BYS$Species), aes(x = Year, y = CatchTon)) +
  ...
  scale_fill_oi(
    ...
    order = 8:1) +
  theme_bw() +
  labs(title="Yearly discards by species in ALF fisheries in the SIOFA area", x="Year", y="Discards (t)") +
  scale_x_continuous(limits=c(2013, 2024),breaks = seq(2013, 2024, by = 5)) +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio=0.4) 

ggsave("BYS summary/discards_species_BYS_web.png", width = 10, height = 4, dpi = 150)

# calculate discard rates
# total catch per year
catch_discards_BYS <- catch_bycatch_subarea_BYS %>% 
  dplyr::select(-SubAreaNo) %>%
  replace(is.na(.), 0) %>%
  group_by(Year) %>%
  summarise(TotCatch=sum(TargetCatch, NonTargetCatch))
# total discards per year
discards_BYS <- discards_species_BYS %>%
  group_by(Year) %>%
  summarise(TotDiscards=sum(CatchTon))
# join and correct for discards double counting
catch_discards_BYS <- full_join(catch_discards_BYS, discards_BYS)
catch_discards_BYS <- catch_discards_BYS %>%
  replace(is.na(.), 0) %>%
...
# colorblind friendly version
ggplot(catch_discards_BYS, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="fill", stat="identity") +
  labs(title="Yearly retained catch/discards in ALF fisheries in the SIOFA area (relative)", x="Year", y="Total catch proportion") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 5:6) +
...
ggsave("BYS summary/SIOFAcatch_discards_BYS_fill_web.png", width = 10, height = 4, dpi = 150)

#colorblind friendly version
ggplot(catch_discards_BYS, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="stack", stat="identity") +
  labs(title="Yearly retained catch/discards in ALF fisheries in the SIOFA area (absolute)", x="Year", y="Total catch (t)") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 4:5) +
...
ggsave("BYS summary/SIOFAcatch_discards_BYS_web.png", width = 10, height = 4, dpi = 150)
...
#colorblind friendl(ier) version
ggplot(yearly_BYS_VME_catch, aes(x = Year, y = Weight, fill=FAOcode)) +
  geom_bar(position="stack", stat="identity") +
  labs(title="Yearly incidental catch of VME indicator taxa in ALF fisheries in the SIOFA area", x="Year", y="Total weight (kg)") +
...
ggplot() +
  ...
  ggtitle("SIOFA incidental captures of seabirds, marine mammals, turtles and 
          sharks considered to be at high risk and/or of concern in ALF target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable 
FT.captures_seabirds_aggregate_BYS <- captures_seabirds_aggregate_BYS %>%
  dplyr::select(1:5) %>%
  flextable() %>%
  ...
  add_header_lines("Observed captures of seabirds in SIOFA alfonsino fisheries") 

save_as_image(FT.captures_seabirds_aggregate_BYS, path = "BYS Summary/Tables/captures_seabirds_BYS.png")               
...
ggplot() +
  ...
  ggtitle("SIOFA observations of seabirds and marine mammals
            in ALF target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable
FT.countries_observing_BYS <- countries_observing_BYS %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Alfonsino observer data submitted by different SIOFA CCPs")

save_as_image(FT.countries_observing_BYS, path = "BYS summary/Tables/countries_observing_BYS.png")  
...
# as flextable
FT.measured_fish_BYS <- measured_fish_BYS %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  compose(j = 1, i = nrow(measured_fish_BYS), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(measured_fish_BYS), j = 1) %>%
  add_header_lines("Alfonsino observer data measurements")

save_as_image(FT.measured_fish_BYS, path = "BYS summary/Tables/measured_fish_BYS.png")  
...
# as flextable
FT.countries_fishing_ALF <- countries_fishing_ALF %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Alfonsino catch and effort data submitted by different SIOFA CCPs")

save_as_image(FT.countries_fishing_ALF, path = "BYS summary/Tables/countries_fishing_ALF.png")  

## analyze average number of vessels in the last 5 years
vessels_fishing_ALF <- fishing %>%
  filter(ActivityID %in% yearly_operations_BYS_target$ActivityID 
         & datasetID %in% yearly_operations_BYS_target$datasetID |
           Target == "BYS") %>%
