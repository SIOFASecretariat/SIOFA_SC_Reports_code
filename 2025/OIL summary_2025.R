...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA OIL/LEC fishing activities (5 degrees, 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
##discards
# filter all OIL target operations (with full data)
OIL_target_operations <- fishing %>%
...
# load discards data
discards_species_OIL <- filter(OIL_target_operations, catchFate == "0")
# plot discards
# sort discards data for plotting
sort_discards_species_OIL <- aggregate(discards_species_OIL$CatchTon, by=list(Species=discards_species_OIL$SpeciesCode), FUN=sum, na.rm = TRUE)
sort_discards_species_OIL <- arrange(sort_discards_species_OIL, desc(x)) 
top5_discards_species_OIL <- sort_discards_species_OIL %>% slice(1:5)
other_discards_species_OIL <- sort_discards_species_OIL %>% slice(6:52)

ggplot(data= subset(discards_species_OIL, SpeciesCode %in% top5_discards_species_OIL$Species), aes(x = Year, y = CatchTon)) +
  geom_bar(aes(fill=SpeciesCode), position="stack", stat="identity") +
  geom_bar(data= subset(discards_species_OIL, SpeciesCode %in% other_discards_species_OIL$Species), aes(x = Year, y = CatchTon, fill="'Other species'"), position="stack", stat="identity") +
  theme_bw() +
...
ggplot(data= subset(discards_species_OIL, SpeciesCode %in% top5_discards_species_OIL$Species), aes(x = Year, y = CatchTon)) +
  ...
  scale_fill_oi(
    ...
    order = 8:1) +
  theme_bw() +
  labs(title="Yearly discards by species in OIL/LEC fisheries in the SIOFA area", x="Year", y="Discards (t)") +
  scale_x_continuous(limits=c(2013, 2024),breaks = seq(2013, 2024, by = 5)) +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio=0.4) 

ggsave("OIL summary/discards_species_OIL_web.png", width = 10, height = 4, dpi = 150)

# calculate discard rates
# total catch per year
catch_discards_OIL <- catch_bycatch_subarea_OIL %>% 
  dplyr::select(-SubAreaNo) %>%
  replace(is.na(.), 0) %>%
  group_by(Year) %>%
  summarise(TotCatch=sum(TargetCatch, NonTargetCatch))
# total discards per year
discards_OIL <- discards_species_OIL %>%
  group_by(Year) %>%
  summarise(TotDiscards=sum(CatchTon))
# join and correct for discards double counting
catch_discards_OIL <- full_join(catch_discards_OIL, discards_OIL)
catch_discards_OIL <- catch_discards_OIL %>%
  replace(is.na(.), 0) %>%
...
# colorblind friendly version
ggplot(catch_discards_OIL, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="fill", stat="identity") +
  labs(title="Yearly retained catch/discards in OIL/LEC fisheries in the SIOFA area (relative)", x="Year", y="Total catch proportion") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 5:6) +
...
ggsave("OIL summary/SIOFAcatch_discards_OIL_fill_web.png", width = 10, height = 4, dpi = 150)

#colorblind friendly version
ggplot(catch_discards_OIL, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="stack", stat="identity") +
  labs(title="Yearly retained catch/discards in OIL/LEC fisheries in the SIOFA area (absolute)", x="Year", y="Total catch (t)") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 4:5) +
...
ggsave("OIL summary/SIOFAcatch_discards_OIL_web.png", width = 10, height = 4, dpi = 150)
...
ggplot() +
  ...
  ggtitle("SIOFA incidental captures of seabirds, marine mammals, turtles and 
          sharks considered to be at high risk and/or of concern in OIL/LEC target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
ggplot() +
  ...
  ggtitle("SIOFA observations of seabirds and marine mammals
            in OIL/LEC target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable
FT.observations_seabirds_aggregate_OIL <- observations_seabirds_aggregate_OIL %>%
  flextable() %>%
  ...
  add_header_lines("Observations of seabirds in OIL/LEC fisheries (2004-2023)") 

save_as_image(FT.observations_seabirds_aggregate_OIL, path = "OIL summary/Tables/seabirds_observations_OIL.png")               
...
# as flextable
FT.countries_observing_OIL <- countries_observing_OIL %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Oilfish observer data submitted by different SIOFA CCPs")

save_as_image(FT.countries_observing_OIL, path = "OIL summary/Tables/countries_observing_OIL.png")  

# measures of maturity, sex and weight, and otoliths collected for OIL
measured_fish_OIL <- Observer_data %>%
  #filter(Year>=2013) %>%
  filter((species3ACode == "OIL")) %>%
  group_by(Year) %>%
...
# as flextable
FT.measured_fish_OIL <- measured_fish_OIL %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  compose(j = 1, i = nrow(measured_fish_OIL), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(measured_fish_OIL), j = 1) %>%
  add_header_lines("Oilfish observer data measurements")

save_as_image(FT.measured_fish_OIL, path = "OIL summary/Tables/measured_fish_OIL.png")  


measured_fish_LEC <- Observer_data %>%
  #filter(Year>=2013) %>%
  filter(species3ACode == "LEC") %>%
  group_by(Year) %>%
  summarise_all(funs(sum(!is.na(.)))) %>%
  dplyr::select(Year, bsLength, bsWeight,bsOtolithCollected,bsSex,bsMaturity,bsGonadWeight,bsStomachSampled) %>%
  rename('Maturity (n)'=bsMaturity) %>%
...
sums_measured_fish_LEC <- colSums(measured_fish_LEC)
measured_fish_LEC <- rbind(measured_fish_LEC,sums_measured_fish_LEC)

write_xlsx(measured_fish_LEC,"OIL summary/Tables/measured_fish_LEC.xlsx")

# as flextable
FT.measured_fish_LEC <- measured_fish_LEC %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 23, unit="cm") %>%
  ...
  flextable::align(j = 1, align = "left", part = "all") %>%
  flextable::align(j = 2:8, align = "center", part = "all") %>%
  compose(j = 1, i = nrow(measured_fish_LEC), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(measured_fish_LEC), j = 1) %>%
  add_header_lines("Escolar observer data measurements")

save_as_image(FT.measured_fish_LEC, path = "OIL summary/Tables/measured_fish_LEC.png")  
...
# as flextable
FT.countries_fishing_OIL <- countries_fishing_OIL %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Oilfish catch and effort data submitted by different SIOFA CCPs")

save_as_image(FT.countries_fishing_OIL, path = "OIL summary/Tables/countries_fishing_OIL.png")  

## analyze average number of vessels in the last 5 years
vessels_fishing_OIL <- fishing %>%
  filter(ActivityID %in% yearly_operations_OIL_target$ActivityID 
         & datasetID %in% yearly_operations_OIL_target$datasetID |
           Target == "OIL/LEC"| Target == "OIL" | Target == "LEC" | Target == "LEC/OIL"
  ) %>%
