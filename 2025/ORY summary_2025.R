...
#roughy_count5 <- roughy_poly5 %>% group_by(id) %>% summarise(n = n())
...
ggplot() +
  ...
  ggtitle("SIOFA ORY fishing activities (hybrid 20'+30', 2014–2023)") +
...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA ORY fishing activities (5 degrees, 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA ORY fishing activities (1 degree, 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
  labs(fill="N. fishing events") +
  annotation_scale(location = "bl", width_hint = 0.1) +
  annotation_north_arrow(location = "tl", which_north = "true", 
...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA ORY fishing activities (30', 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
  labs(fill="N. fishing events") +
  annotation_scale(location = "bl", width_hint = 0.1) +
  annotation_north_arrow(location = "tl", which_north = "true", 
...
ggplot() +
  ...
  coord_sf(crs = "4326") +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("SIOFA ORY fishing activities (20', 2014–2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
  labs(fill="N. fishing events") +
  annotation_scale(location = "bl", width_hint = 0.1) +
  annotation_north_arrow(location = "tl", which_north = "true", 
...
sums_MU_fish_ORY <- colSums(catch_bycatch_by_ORY_MU_table, na.rm=TRUE)
catch_bycatch_by_ORY_MU_table <- rbind(catch_bycatch_by_ORY_MU_table,sums_MU_fish_ORY)

write_xlsx(catch_bycatch_by_ORY_MU_table,"ORY summary/Tables/catch_bycatch_by_ORY_MU_table.xlsx")

# as flextable
FT.catch_bycatch_by_ORY_MU_table <- catch_bycatch_by_ORY_MU_table %>%
  flextable() %>%
  ...
  compose(j = 1, i = nrow(catch_bycatch_by_ORY_MU_table), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(catch_bycatch_by_ORY_MU_table), j = 1) %>%
  add_header_lines("Orange roughy catch per year and per assessment area")

save_as_image(FT.catch_bycatch_by_ORY_MU_table, path = "ORY summary/Tables/catch_bycatch_by_ORY_MU_table.png")  
...
##discards
# filter all ORY target operations (with full data)
ORY_target_operations <- fishing %>%
  filter(ActivityID %in% yearly_operations_ORY_target$ActivityID 
         & datasetID %in% yearly_operations_ORY_target$datasetID |
           Target == "ORY")
# load discards data
discards_species_ORY <- filter(ORY_target_operations, catchFate == "0")
# plot discards
# sort discards data for plotting
sort_discards_species_ORY <- aggregate(discards_species_ORY$CatchTon, by=list(Species=discards_species_ORY$SpeciesCode), FUN=sum, na.rm = TRUE)
sort_discards_species_ORY <- arrange(sort_discards_species_ORY, desc(x)) 
top5_discards_species_ORY <- sort_discards_species_ORY %>% slice(1:5)
other_discards_species_ORY <- sort_discards_species_ORY %>% slice(6:52)

ggplot(data= subset(discards_species_ORY, SpeciesCode %in% top5_discards_species_ORY$Species), aes(x = Year, y = CatchTon)) +
  geom_bar(aes(fill=SpeciesCode), position="stack", stat="identity") +
  geom_bar(data= subset(discards_species_ORY, SpeciesCode %in% other_discards_species_ORY$Species), aes(x = Year, y = CatchTon, fill="'Other species'"), position="stack", stat="identity") +
  theme_bw() +
...
ggplot(data= subset(discards_species_ORY, SpeciesCode %in% top5_discards_species_ORY$Species), aes(x = Year, y = CatchTon)) +
  ...
  scale_fill_oi(
    ...
    order = 8:1) +
  theme_bw() +
  labs(title="Yearly discards by species in ORY fisheries in the SIOFA area", x="Year", y="Discards (t)") +
  scale_x_continuous(limits=c(2013, 2024),breaks = seq(2013, 2024, by = 5)) +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio=0.4) 

ggsave("ORY summary/discards_species_ORY_web.png", width = 10, height = 4, dpi = 150)

# calculate discard rates
# total catch per year
catch_discards_ORY <- catch_bycatch_subarea_ORY %>% 
  dplyr::select(-SubArea) %>%
  replace(is.na(.), 0) %>%
  group_by(Year) %>%
  summarise(TotCatch=sum(TargetCatch, NonTargetCatch))
# total discards per year
discards_ORY <- discards_species_ORY %>%
  group_by(Year) %>%
  summarise(TotDiscards=sum(CatchTon))
# join and correct for discards double counting
catch_discards_ORY <- full_join(catch_discards_ORY, discards_ORY)
catch_discards_ORY <- catch_discards_ORY %>%
  replace(is.na(.), 0) %>%
...
# colorblind friendly version
ggplot(catch_discards_ORY, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="fill", stat="identity") +
  labs(title="Yearly retained catch/discards in ORY fisheries in the SIOFA area (relative)", x="Year", y="Total catch proportion") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 5:6) +
...
ggsave("ORY summary/SIOFAcatch_discards_ORY_fill_web.png", width = 10, height = 4, dpi = 150)

#colorblind friendly version
ggplot(catch_discards_ORY, aes(x = Year, y = t, fill=Catch)) +
  geom_bar(position="stack", stat="identity") +
  labs(title="Yearly retained catch/discards in ORY fisheries in the SIOFA area (absolute)", x="Year", y="Total catch (t)") +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 4:5) +
...
ggsave("ORY summary/SIOFAcatch_discards_ORY_web.png", width = 10, height = 4, dpi = 150)
...
# as flextable
FT.countries_fishing_ORY <- countries_fishing_ORY %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  add_header_lines("Orange roughy catch and effort data submitted by different SIOFA CCPs")

save_as_image(FT.countries_fishing_ORY, path = "ORY summary/Tables/countries_fishing_ORY.png")  

## analyze average number of vessels in the last 5 years
vessels_fishing_ORY <- fishing %>%
  filter(ActivityID %in% yearly_operations_ORY_target$ActivityID 
         & datasetID %in% yearly_operations_ORY_target$datasetID |
           Target == "ORY") %>%
...
#top5 VME bycatch
sort_VME <- aggregate(yearly_ORY_VME_catch$Weight, by=list(Species=yearly_ORY_VME_catch$FAOcode), FUN=sum, na.rm = TRUE)

sort_VME <- arrange(sort_VME, desc(x)) 

top5_VME <- sort_VME %>% slice(1:5)
other_VME <- sort_VME %>% slice(6:26)

ggplot(data= subset(yearly_ORY_VME_catch, FAOcode %in% top5_VME$Species), aes(x = Year, y = Weight)) +
  geom_bar(aes(fill=FAOcode), position="stack", stat="identity") +
  geom_bar(data= subset(yearly_ORY_VME_catch, FAOcode %in% other_VME$Species), aes(x = Year, y = Weight, fill="'Other species'"), position="stack", stat="identity") +
  theme_bw() +
  scale_x_continuous(limits=c(2003, 2024),breaks = seq(2003, 2024, by = 5)) +
  scale_fill_oi(
    palette = "full",
    reverse = FALSE,
    order = 8:1) +
  labs(title="Yearly incidental catch of VME indicator taxa in ORY fisheries in the SIOFA area", x="Year", y="Discards (t)") +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio=0.8)

ggsave("ORY summary/VME_captures_ORY_Top5_web.png", width = 10, height = 6, dpi = 150) 
...
ggplot() +
  ...
  ggtitle("SIOFA incidental captures of seabirds, marine mammals, turtles and 
          sharks considered to be at high risk and/or of concern in ORY target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable 
FT.captures_sharks_aggregate_ORY <- captures_sharks_aggregate_ORY %>%
  dplyr::select(1:5) %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  ...
  add_header_lines("Observed captures of sharks at “high risk” and/or “of concern” in SIOFA orange roughy fisheries") 

save_as_image(FT.captures_sharks_aggregate_ORY, path = "ORY Summary/Tables/captures_sharks_ORY.png")               
...
ggplot() +
  ...
  ggtitle("SIOFA observations of seabirds and marine mammals
            in ORY target operations (2004-2023)") +
  coord_sf(xlim = c(25, 125), ylim = c(15, -60), expand = FALSE) +
...
# as flextable
FT.countries_observing_ORY <- countries_observing_ORY %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 18, unit="cm") %>%
  ...
  add_header_lines("Orange roughy observer data submitted by different SIOFA CCPs")

save_as_image(FT.countries_observing_ORY, path = "ORY summary/Tables/countries_observing_ORY.png")  
...
# as flextable
FT.measured_fish_ORY <- measured_fish_ORY %>%
  flextable() %>%
  bg(bg = "grey", part = "header") %>%
  autofit(part = c("body")) %>%
  fit_to_width(max_width = 23, unit="cm") %>%
  ...
  compose(j = 1, i = nrow(measured_fish_ORY), as_paragraph(as_chunk('Total'))) %>%
  bold(i = nrow(measured_fish_ORY), j = 1) %>%
  add_header_lines("Orange roughy observer data measurements")

save_as_image(FT.measured_fish_ORY, path = "ORY summary/Tables/measured_fish_ORY.png")  
